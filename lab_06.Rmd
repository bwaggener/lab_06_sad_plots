...
ggplot(continent_totals, aes(
  x = reorder(continent, -total_production),  # Order continents by total production
  y = total_production,
  fill = factor(type)  # Fill bars with different colors based on 'type'
)) +
  geom_bar(stat = "identity", position = "stack") +  # Stack the bars
  scale_fill_viridis_d(option = "viridis") +  # Use a distinct color palette for fill
  labs(
    title = "Fishery Production by Continent",
    x = "Continent",
  ...
  theme(
    ...
    plot.margin = margin(1, 1, 1, 3)  # Adjust margins to make space for the labels
  )
```
